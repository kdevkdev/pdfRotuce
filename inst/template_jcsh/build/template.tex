...
\abstractmainlangtitle{$string_abstract_mainlang_title$}
...
\articledates{
$for(articledates)$$articledates$${ sep }; $endfor$ % received, editorial decision, published .. .
} % received, editorial decision, published .. .
...
\fancyhead[LE]{$author_shortname$}
\fancyhead[RO]{$journalinfo.title_short$}
...
\fancyfoot[LE]{%
	\begin{textblock}{1}[0.5,0.5](1.5,14.5){\rotatebox{90}{$author_shortname$}}\end{textblock}}
\fancyfoot[LO] {%
	\begin{textblock}{1}[0.5,0.5](1.5,14.5){\rotatebox{90}{$journalinfo.title_short$}}\end{textblock}}
...
\captionof{figure}{{\bfseries\sffamily #1.} \sffamily\raggedright #3}}% provide customized nmbering. figure name and nmber disabled in caption package optoins
{\endminipage\par\medskip}

\newenvironment{FigureMW}[3]
{\begin{figure*}
\centering
\includegraphics[width=1\textwidth]{#2}
\captionof{figure}{{\bfseries\sffamily #1.} \sffamily\raggedright #3}}
{\end{figure*}}
...
%\NewDocumentEnvironment{dienquote}{m}	
%{\begin{displayquote}\begin{enquote}{\textit}}
%{{\mdseries#1}\end{enquote}\end{displayquote}}
...
\\
\noindent
{\fontsize{10}{12}\selectfont\setstretch{1.14}{\bfseries\color{color1}$string_keywords_heading$} $for(keywords)$$keywords$$sep$, $endfor$}$endif$
\end{abstract}
$endif$
...
\begin{multicols}{2}
  \raggedcolumns
	...
	$if(statements)$\section{$string_declarations_title$}$endif$
	$for(statements)$
\subsection{$statements.title$}
\bgroup \setlength{\parindent}{0pt}$statements.text$
\egroup

	$endfor$
	
	$if(orcids)$\subsection{ORCIDs}$endif$
\bgroup \setlength{\parindent}{0pt}
	$for(orcids)$
$orcids.name$\orcidaffil{$orcids.orcid$} \href{https://orcid.org/$orcids.orcid$}{$orcids.orcid$}\\
	$endfor$
\egroup
\end{multicols}
...
$if(abstracts.sidelangs)$ 
\section{$string_multilang_abstract_title$}
\vspace{2mm}
%$--${ partials/multilang_abstracts.tex() }
$for(abstracts.sidelangs)$
\begin{tcolorbox}[colframe=themecolor2, colback=themecolor2, sharp corners,boxsep=4mm,top=3.5mm,left=3.8mm,right=2.0mm,bottom=1mm,breakable]\sffamily
	{\fontsize{11}{14}\selectfont \bfseries $abstracts.sidelangs.title$} 
	\vskip 1mm
	$for(abstracts.sidelangs.parts)$
        	{\bfseries $abstracts.sidelangs.parts.title$} $abstracts.sidelangs.parts.text$ $sep$\\ 
    $endfor$
$if(abstracts.sidelangs.keywords)$\vskip 3mm{\raggedright\bfseries $abstracts.sidelangs.string_keywords_heading$}  $abstracts.sidelangs.keywords$ $endif$
\end{tcolorbox}
\vspace{3mm}
$endfor$
$endif$
